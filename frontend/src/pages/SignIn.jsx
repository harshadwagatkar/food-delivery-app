...
export default function SignIn() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${serverURL}/api/auth/signin`, user, 
  ...
  const handleGoogleAuth = async (e) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.msg || "Google sign-in failed");
    }
  };
      ...
      <div className="w-[360px] bg-white rounded-xl shadow-lg px-6 py-5">
        ...
        <form className="space-y-4" action="" onSubmit={handleSubmit}>
          ...
          <div className="flex justify-end">
            <Link
              to="/forgot-password"
              className="text-sm font-medium text-[#F94A29] hover:underline"
            >
              Forgot password?
            </Link>
          </div>
